...
class InferenceState:
    ...
    def stack_finished(self):
        ...
        self.finished = T.stack(self.finished)

    def pad_to_max(self, seq, max_len:int):
        ...
        if self.gumbel:
            ...
            eps = 1e-8
            pads = T.zeros(max_len - len(seq), len(self.vocab)).to(seq.device)
            pads = pads.fill_(eps).index_fill_(1, T.tensor(self.pad_idx), 1.)
        else:
            pads = T.zeros(max_len - len(seq)).fill_(self.pad_idx).to(seq.device)
